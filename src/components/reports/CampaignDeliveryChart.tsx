...
export function CampaignDeliveryChart({ data }: Props) {
  ...
  if (data.length === 0) {
    return <div className="h-64 flex items-center justify-center text-slate-400 text-sm border border-dashed border-slate-200 rounded-lg">{t('reports.chart.noData')}</div>;
  }
    ...
    <div className="w-full h-72 flex flex-col">
          ...
          return (
            <div key={i} className="flex-1 flex flex-col items-center justify-end h-full z-10 group relative">
              <div className="absolute -top-16 opacity-0 group-hover:opacity-100 transition-opacity bg-slate-800 text-white text-xs rounded py-1.5 px-3 whitespace-nowrap shadow-xl pointer-events-none z-20">
                ...
                <div>{formatNumber(day.plays)} {t('reports.chart.plays')}</div>
                <div className="text-indigo-300">{formatNumber(day.estimatedImpressions)} {t('reports.chart.estImp')}</div>
                <div className="absolute -bottom-1 left-1/2 -translate-x-1/2 w-2 h-2 bg-slate-800 rotate-45" />
              </div>
              <div className="w-full max-w-[40px] flex items-end space-x-0.5 relative h-full">
                <div className="flex-1 bg-slate-300 rounded-t-sm hover:bg-slate-400 transition-colors" style={{ height: `${playsHeight}%` }} />
                <div className="flex-1 bg-indigo-500 rounded-t-sm hover:bg-indigo-600 transition-colors" style={{ height: `${impressionsHeight}%` }} />
              </div>
            </div>
          );
        })}
      ...
      <div className="flex justify-center items-center space-x-6 mt-4 pt-4 border-t border-slate-100">
        <div className="flex items-center text-xs text-slate-600 font-medium">
          ...
          {t('reports.chart.legendPlays')}
        </div>
        <div className="flex items-center text-xs text-slate-600 font-medium">
          <div className="w-3 h-3 rounded-sm bg-indigo-500 mr-2" />
          {t('reports.chart.legendImp')}
        </div>
      </div>
    </div>
  );
