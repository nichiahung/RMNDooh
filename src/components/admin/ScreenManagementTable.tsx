...
export function ScreenManagementTable({ screens, inventory }: Props) {
  ...
  const getStatusBadge = (status: Screen['status']) => {
    switch (status) {
      case 'online':
        return <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded text-xs font-bold border border-emerald-200">{t('admin.screens.statusOnline')}</span>;
      case 'offline':
        return <span className="text-red-600 bg-red-50 px-2 py-0.5 rounded text-xs font-bold border border-red-200">{t('admin.screens.statusOffline')}</span>;
      case 'maintenance':
        return <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded text-xs font-bold border border-amber-200">{t('admin.screens.statusMaintenance')}</span>;
    }
  };

  const getTimeAgo = (dateStr: string) => {
    const ms = new Date().getTime() - new Date(dateStr).getTime();
    if (ms < 60000) return t('admin.screens.justNow');
    if (ms < 3600000) return `${Math.floor(ms/60000)}${t('admin.screens.minutesAgo')}`;
    if (ms < 86400000) return `${Math.floor(ms/3600000)}${t('admin.screens.hoursAgo')}`;
    return `${Math.floor(ms/86400000)}${t('admin.screens.daysAgo')}`;
  };
    ...
    <div className="flex flex-col h-full">
      ...
      <div className="p-4 border-b border-slate-200 flex items-center justify-between bg-white">
        <div className="relative w-64">
          ...
          <input
            type="text"
            placeholder={t('admin.screens.searchPlaceholder')}
        ...
        <div className="flex space-x-2">
          <button className="flex items-center px-4 py-2 bg-white border border-slate-300 text-slate-700 font-medium rounded-lg text-sm shadow-sm hover:bg-slate-50 transition-colors">
            <RotateCw className="w-4 h-4 mr-2" /> {t('admin.screens.refreshStatus')}
          </button>
        </div>
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="text-xs text-slate-500 uppercase tracking-wider bg-slate-50 border-b border-slate-200">
            <tr>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.screenId')}</th>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.location')}</th>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.status')}</th>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.lastHeartbeat')}</th>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.currentPlayback')}</th>
              <th className="px-6 py-4 font-semibold">{t('admin.screens.col.specs')}</th>
            </tr>
          </thead>
            ...
            {screens.map(screen => {
              ...
              return (
                <tr key={screen.screenId} className={`transition-colors ${isOffline ? 'bg-red-50/30' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4">{getStatusBadge(screen.status)}</td>
                  <td className="px-6 py-4">
                  ...
                  <td className="px-6 py-4">
                    {screen.status === 'online' ? (
                      screen.currentCampaignId ? (
                        ...
                        <span className="text-slate-400 italic">{t('admin.screens.defaultLoop')}</span>
                      )
                    ) : (
