...
export function InventoryManagementTable({ inventory }: Props) {
  ...
  const getAvailabilityBadge = (av: number) => {
    if (av >= 0.7) return <span className="text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded text-xs font-semibold">High ({Math.round(av*100)}%)</span>;
    if (av >= 0.3) return <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded text-xs font-semibold">Limited ({Math.round(av*100)}%)</span>;
    return <span className="text-slate-600 bg-slate-100 px-2 py-0.5 rounded text-xs font-semibold">Low ({Math.round(av*100)}%)</span>;
  };
        ...
        <table className="w-full text-sm text-left whitespace-nowrap">
          <thead className="text-xs text-slate-500 uppercase tracking-wider bg-slate-50 border-b border-slate-200">
            <tr>
              ...
              <th className="px-6 py-4 font-semibold text-right">Daily Imp.</th>
